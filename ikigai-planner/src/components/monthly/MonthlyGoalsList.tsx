...
export function MonthlyGoalsList({ userId, year, month }: MonthlyGoalsListProps) {
  const { goals, isLoading, createGoal, updateGoal, toggleComplete, deleteGoal } =
    useMonthlyGoals(userId, year, month)
      ...
      {isLoading ? (
        ...
        <ul className="space-y-2">
            ...
            return (
              ...
              >
                ...
                {!isCompleted && goal.progress > 0 && (
                  <span className="text-xs px-1.5 py-0.5 rounded-full bg-blue-100 text-blue-700 font-medium flex-shrink-0">
                    {goal.progress}%
                  </span>
                )}
          ...
          {goals.length === 0 && !showInput && (
            <li className="text-sm text-gray-400 italic px-2">Sin metas este mes</li>
          )}
